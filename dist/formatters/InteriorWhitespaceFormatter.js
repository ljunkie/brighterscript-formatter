...
class InteriorWhitespaceFormatter {
    ...
    format(tokens, parser, options) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            // Don't modify single `]` on its own line
    ...
    */
    formatTokenSpacing(tokens, parser, options) {
        ...
        for (i; i < tokens.length; i++) {
            ...
            if (!options.insertSpaceBetweenFunctionAssignments && (token.kind === brighterscript_1.TokenKind.Sub || token.kind === brighterscript_1.TokenKind.Function)) {
                ...
                if (parenToken) {
                    ...
                    while (j < tokens.length && parenDepth > 0) {
                        ...
                        if (t.kind === brighterscript_1.TokenKind.LeftParen) {
                            parenDepth++;
                        }
                        ...
                        if ([brighterscript_1.TokenKind.Equal, brighterscript_1.TokenKind.PlusEqual, brighterscript_1.TokenKind.MinusEqual,
                            brighterscript_1.TokenKind.StarEqual, brighterscript_1.TokenKind.ForwardslashEqual, brighterscript_1.TokenKind.BackslashEqual].includes(t.kind)) {
